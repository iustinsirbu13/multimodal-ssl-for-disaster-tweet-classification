...
class ImageBertEmbeddings(nn.Module):
    ...
    def forward(self, input_imgs, token_type_ids):
        ...
        cls_id = torch.LongTensor([self.args.vocab.stoi["[CLS]"]]).to(self.args.device)
        ...
        position_ids = torch.arange(seq_length, dtype=torch.long).to(self.args.device)
...
class MultimodalBertEncoder(nn.Module):
    ...
    def forward(self, input_txt, attention_mask, segment, input_img):
        ...
        attention_mask = torch.cat(
            [
                torch.ones(bsz, self.args.num_image_embeds + 2).long().to(self.args.device),
                attention_mask,
            ],
            dim=1,
        )
